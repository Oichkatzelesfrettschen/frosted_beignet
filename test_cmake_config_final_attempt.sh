...
echo "--- Attempting to configure the project (CMake only) after commenting out -fno-rtti (Final Attempt for this step) ---"

BUILD_DIR="build_cmake_final_attempt" # New, distinct build directory
...
# Using timeout to prevent indefinite hanging, if that's an issue
timeout 300s cmake .. > >(tee cmake_output.log) 2> >(tee cmake_error.log >&2)
CMAKE_EXIT_CODE=${PIPESTATUS[0]}

if [ $CMAKE_EXIT_CODE -eq 124 ]; then
    echo "CMake configuration timed out after 5 minutes."
    exit 124
elif [ $CMAKE_EXIT_CODE -ne 0 ]; then
